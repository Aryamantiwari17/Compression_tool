...
protected:
    string sname; // these are private data members with acurate naming convention
    float fratio;
    float fspeed;
...
public:
    ...
    {
        this->sname = name;// access the member variables of the instance.
        this->fratio = ratio;
        this->fspeed = speed;
    }
    ...
    float getCompressionRatio(float foriginalSize, float fcompressedSize)
    {
        // calculate compression ratio
        float fcompressionRatio = (originalSize / compressedSize);//calculating the compression ratio

        // Output compression ratio
        cout << "Compression Ratio: " << fcompressionRatio << ":1" << endl;

        return compressionRatio;
    }

    float getCompressionSpeed(float ffileSize, float fduration)//calculTING THE COMPREESION SPEED
    {
        // Calculate compression speed
        float fcompressionSpeed = fileSize / (duration / 1000.0); // in MB/s

        // Output compression speed
        cout << "Compression Speed: " << fcompressionSpeed << " MB/s" << endl;

        return fcompressionSpeed;
    }
};
...
int main()
{
   // Building the main function-Written By 211130-AryamanTiwari
    ...
    cin >> fspeed;
    CCCompressionAlgorithm *algo = new CCCompressionAlgorithm(strname, fratio, fspeed);
    algo->getName();
